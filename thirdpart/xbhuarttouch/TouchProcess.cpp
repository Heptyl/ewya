...
XBH_S32 setSourceTouchState(XBH_BOOL enable)
{
    ...
    buf[6] = 0x00;
    buf[7] = 0x00;
    buf[8] = enable ? 0x02 : 0x01;
    buf[9] = CalcCheckSum(buf, 9);

    ret = sendCommand(buf, 10);
    if(ret == XBH_SUCCESS)
    {
...
{
    XBH_U8 TouchScalePackage[9] = {0x1F,0xF7,0xFC,0xB4,0x06,0x01,0x00,0x00,0x00};
    if(resolution == 0)
    {
        //if the ui is 2k
        switch (y) {
