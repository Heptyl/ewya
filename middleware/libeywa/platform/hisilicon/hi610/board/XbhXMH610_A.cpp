...
//第1次ps_on信号低电平持续时间
const  XBH_U32           XbhXMH610_A::M_PS_ON_BASE_DELAY_TIME(700);
//低电平时长递增步长
const  XBH_U32           XbhXMH610_A::M_PS_ON_DELAY_TIME_STEP(200);
//不同低电平间隔时间
const  XBH_U32           XbhXMH610_A::M_PWR_STA_DECT_TIMES(3000);
...
void* XbhXMH610_A::setOpsPowerCtrlTask(void* arg)
{
    XbhXMH610_A* self = static_cast<XbhXMH610_A*>(arg);
...
XbhXMH610_A::XbhXMH610_A()
{
    ...
    //如果是OPS通过网络唤醒610，由于OPS的CRC做到了uboot，没有办法进行CRC校验，所以如果启动的时候读取到这次启动时被OPS唤醒的，默认认为该通道是CRC校验通过的通道。
    XBH_WAKEUP_S stWake;
    getWakeUpReason(&stWake);
    if (XBH_WAKEUP_SOURCE == stWake.enWakeup)
    {
        ...
        if (stWake.enSrc == XBH_SOURCE_OPS1)
        {
            XLOGD("Wake up through OPS1");
            property_set("vendor.xbh.ops1crc.result", result_status);
        }
        else if (stWake.enSrc == XBH_SOURCE_OPS2)
        {
            XLOGD("Wake up through OPS2");
...
XBH_S32 XbhXMH610_A::getOpsWakeSoc(XBH_BOOL *bEnable, XBH_SOURCE_E enSource)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 XbhXMH610_A::getOpsOperationStatus(XBH_BOOL *bEnable, XBH_SOURCE_E enSource)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 XbhXMH610_A::setOpsRecovery(XBH_BOOL bEnable, XBH_SOURCE_E enSource)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    if (enSource == XBH_SOURCE_OPS1) 
    {
...
XBH_S32 XbhXMH610_A::getOps2SignalStatus(XBH_S32 *status)
{
    return this->XbhHi_610::getSignalStatus(XBH_SOURCE_OPS2,status);
}

XBH_S32 XbhXMH610_A::getFUsbc1SignalStatus(XBH_S32 *status)
{
    return this->XbhHi_610::getSignalStatus(XBH_SOURCE_F_USBC1,status);
}

XBH_S32 XbhXMH610_A::getFHdmi1SignalStatus(XBH_S32 *status)
{
...
XBH_S32 XbhXMH610_A::getEdidInvaild(XBH_HDMIRX_EDID_TYPE_E edidType, XBH_U8 *edidInvaild)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 i = 0;
...
XBH_S32 XbhXMH610_A::getEdidModelName(std::string &pBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 i = 0;
    XBH_S32 begin_index = -1;
...
XBH_S32 XbhXMH610_A::getEdidSn(std::string &pBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 i = 0;
    char buff[5];
...
XBH_S32 XbhXMH610_A::setLedPwrStatus(XBH_LED_LIGHT_E enState)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 XbhXMH610_A::getLedPwrStatus(XBH_LED_LIGHT_E* enState)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 XbhXMH610_A::getOPSBypassDataLen(XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 XbhXMH610_A::SendOPSBypassData(XBH_SOURCE_E type, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 XbhXMH610_A::OPSCRCBypassData(XBH_SOURCE_E type, XBH_BOOL RWflag, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_U8 XbhXMH610_A::CalcCRC8(XBH_U8* Buffer, XBH_U8 BufferLen)
{
    XBH_U8* data = Buffer;
    XBH_U16 crc = 0;
    int i, j;
...
XBH_U16 XbhXMH610_A::crc16(XBH_U16 crc, XBH_U8* buffer, XBH_U8 size)
{
    XBH_U8 i = 0;
    for (; i < size; i++)
    {
...
XBH_U16 XbhXMH610_A::Crc16CCIT(XBH_U8* data, XBH_U8 size)
{
    XBH_U16 crc = 0;
...
XBH_S32 XbhXMH610_A::OPSByPassProcess()
{
    XBH_U8 header[2] = {0};
...
//CRC超时检测
XBH_S32 XbhXMH610_A::CRCTimeoutCheck()
{
...
*/
XBH_S32 XbhXMH610_A::setOpsPowerCtrlWithDurationMs(XBH_SOURCE_E enSource)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
//override
XBH_S32 XbhXMH610_A::setReferRTCInfo(const XBH_CHAR* pBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
//override
XBH_S32 XbhXMH610_A::getReferRTCInfo(XBH_CHAR* pBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 XbhXMH610_A::getUSBCForwardReverseInsertionDet(XBH_SOURCE_E idx, XBH_S32 *u32Value)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    switch (idx)
    {
...
//override
XBH_S32 XbhXMH610_A::setSecurityHvbKey()
{
    XBH_S32 s32Ret = XBH_FAILURE;
 ...
 * 设置type-c复位VBUS
*/
XBH_S32 XbhXMH610_A::setTypecReset(XBH_SOURCE_E src)
{
