...
//------------------------------------------------------------------------------
extern uint32_t lvdstx_set_video_format_wo_tx_link(lvdstx_video *p_video);
//------------------------------------------------------------------------------
// lvdstx_set_sg_format
//   sets the supported video format as signal generator for LVDS Tx
//
//   Prototype:
//     uint32_t lvdstx_set_sg_format(lvdstx_sg_video *p_video)
//
//   Parameters:
//     p_video    Pointer to structure of lvdstx_sg_video to hold video format property
//
//   Returns:
//     ERR_OK if successful, otherwise a system error code depending on the function.
//
//   Description:
//     This function sets the video format to use as a signal generator for LVDS Tx.
//     Because the video signal for LVDS Tx is output by using the video signal
//     generated by hdmitx_set_sg_format() function, it must be performed beforehand.
//     This function must be called after lvdstx_init function is called.
...
uint32_t lvdstx_set_sg_format(lvdstx_sg_video *p_video)
{
	...
	vf.rx_port.port_type = SYSTEM_PORT_TYPE_LVDS;		// for lvdstx_hvpol_setting()
	...
	ret = lvdstx_set_video_format_wo_tx_link(&vf);
	CHECK_ERROR(ret);
