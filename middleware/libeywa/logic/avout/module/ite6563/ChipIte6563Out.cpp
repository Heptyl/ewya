#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@ChipIte6563Out"
#include "XbhLog.h"
 ...
 * 复位当前转换芯片
 */
XBH_S32 ChipIte6563Out::resetChip()
{
 ...
 * value 高低状态
 */
XBH_S32 ChipIte6563Out::setRxHpd(XBH_U8 value)
{
 ...
 * 获取chip是否初始化完成
*/
XBH_S32 ChipIte6563Out::getChipInitDone(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}
 ...
 * 获取chip是否存在
*/
XBH_S32 ChipIte6563Out::getChipExist(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}
 ...
 * 获取固件版本
 */
XBH_S32 ChipIte6563Out::getFirmwareVersion(XBH_CHAR* strVersion)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8  u8WriteCompleteFlag[8]={0x55, 0x44, 0xC3, 0x65, 0x16, 0xA0, 0x1A, 0x59};
    XBH_U8  u8ReadCompleteFlag[8]={0};
    ...
    do{
        ...
        s32Ret |= readI2cByteArray(0x2060, sizeof(u8ReadCompleteFlag), (XBH_U8 *)u8ReadCompleteFlag);
        if (s32Ret != XBH_SUCCESS)
        {
            break;
        }
        XLOGD("pu8TempData:0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x ", u8ReadCompleteFlag[0],
                u8ReadCompleteFlag[1], u8ReadCompleteFlag[2],
                u8ReadCompleteFlag[3], u8ReadCompleteFlag[4],
                u8ReadCompleteFlag[5], u8ReadCompleteFlag[6],
                u8ReadCompleteFlag[7]);

        if (memcmp(u8ReadCompleteFlag, u8WriteCompleteFlag, sizeof(u8WriteCompleteFlag)) != 0)
        {
            strcpy(strVersion, "V000");
            XLOGE("firmware update failed!!");
            break;
        }
        ...
        // 找到第一个换行符的位置并截断
        strVersion[strcspn(strVersion, "\r\n")] = '\0';

        XLOGD("[%s,%d] Version: %s", __FUNCTION__, __LINE__, strVersion);
        property_set("persist.vendor.xbh.ite6563.board.ver", strVersion);
    }while(0);
 ...
 * 进行固件升级
 */
XBH_S32 ChipIte6563Out::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    property_set("persist.vendor.xbh.ite6563.board.ver", "");
 ...
 * 获取当前升级的进度
 */
XBH_S32 ChipIte6563Out::getUpgradeState(XBH_S32 * s32State)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    *s32State = mState;
    return s32Ret;
...
XBH_U32 ChipIte6563Out::genCrc32(XBH_VOID* pBuff, XBH_S32 s32Size)
{
...
XBH_S32 ChipIte6563Out::writeI2cByte(XBH_S32 s32RegAddr, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (pu8Data == NULL)
    {
...
XBH_S32 ChipIte6563Out::readI2cByte(XBH_S32 s32RegAddr, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (pu8Data == NULL)
    {
...
XBH_S32 ChipIte6563Out::writeI2cByteArray(XBH_U32 u32Offset, XBH_U32 u32Length, XBH_U8* pu8Buff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    while (u32WriteTotalLength < u32Length)
    {
        ...
        if (s32Ret != XBH_SUCCESS)
        {
            XLOGE("Error: writeI2cByteArray fail!");
            return s32Ret;
        }
...
XBH_S32 ChipIte6563Out::readI2cByteArray(XBH_U32 u32Offset, XBH_U32 u32Length, XBH_U8* pu8Buff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U32 u32CurOffset = 0;
    XBH_S32 s32RegAddr = 0;
    ...
    while (u32ReadTotalLength < u32Length)
    {
        ...
        if (s32Ret != XBH_SUCCESS)
        {
            XLOGE("Error: readI2cByteArray fail!");
            return s32Ret;
        }
...
XBH_S32 ChipIte6563Out::enterIspMode()
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 s32RegAddr = 0;
    XBH_U8  pu8Data[1] = {0};
...
XBH_S32 ChipIte6563Out::exitIspMode()
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 s32RegAddr = 0;
    XBH_U8  pu8Data[1] = {0};
...
XBH_S32 ChipIte6563Out::enableInterruptMask()
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 s32RegAddr = 0;
    XBH_U8  pu8Data[1] = {0};
...
XBH_S32 ChipIte6563Out::checkHead(XBH_U8* pu8Buff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 ChipIte6563Out::eraseSector(XBH_U32 u32WriteOffset, XBH_U32 u32WriteEnd)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 s32RegAddr = 0;
    XBH_U8  pu8Data[3] = {0};
...
XBH_S32 ChipIte6563Out::writeData(XBH_U32 u32Offset, XBH_U32 u32Length, XBH_U8* pu8Buff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 ChipIte6563Out::verifyData(XBH_U32 u32Offset, XBH_U32 u32Length, XBH_U8* pu8Buff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    while (u32ReadTotalLength < u32Length)
    {
        ...
        s32Ret |= readI2cByteArray(u32Offset + u32ReadTotalLength, u32ReadLength, pu8TempData);
        if (s32Ret != XBH_SUCCESS)
        {
            XLOGE("Error: readI2cByteArray fail!");
            return s32Ret;
...
void ChipIte6563Out::run(const void* arg)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    XbhService::getPlatformInterface()->setHdmiTxInfo(XBH_HDMI_TX_VIDEO_MUTE, 1);//关闭 DP
    usleep(1000 * 1000);
    ...
    mState = XBH_UPGRADE_SUCCESS;
    XbhService::getPlatformInterface()->setHdmiTxInfo(XBH_HDMI_TX_VIDEO_MUTE, 0);//打开 DP
}
//---------------- custom device interface end ----------------

ChipIte6563Out::ChipIte6563Out(XBH_U32 i2cBus, XBH_U32 Addr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel)
{
    XLOGD(" begin ");
    ...
    // usleep(100 * 1000);
    // getFirmwareVersion(mVersion);
    // property_set("persist.vendor.xbh.ite6563.board.ver", mVersion);

    XLOGD(" end ");
}

ChipIte6563Out::~ChipIte6563Out()
{
