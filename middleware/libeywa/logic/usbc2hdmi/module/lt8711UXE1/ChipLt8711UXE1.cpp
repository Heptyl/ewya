...
#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@ChipLt8711UXE1"
#include "XbhLog.h"
 ...
 * 复位当前转换芯片
 */
XBH_S32 ChipLt8711UXE1::resetChip()
{
 ...
 * value 高低状态
 */
XBH_S32 ChipLt8711UXE1::setRxHpd(XBH_U8 value)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}
 ...
 * 设置type-c功率
 */
XBH_S32 ChipLt8711UXE1::setTypecCapacity(XBH_U32 value)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    if (mState == LT8711_RUNNING)
    {
        XLOGW("Warning: lt8711 is updating, stop setTypecCapacity !!!");
        return s32Ret;
    }
    if (value == 0) 
    {
        s32Ret = i2c_data_write(0xFF, 0xE0);
        s32Ret |= i2c_data_write(0xB2, 0x01);
    } 
    else if (value == 1)
    {
        s32Ret = i2c_data_write(0xFF, 0xE0);
        s32Ret |= i2c_data_write(0xB2, 0x00);
    }
    return s32Ret;
}
/**
 * 获取固件版本
 */
XBH_S32 ChipLt8711UXE1::getFirmwareVersion(XBH_CHAR* strVersion)
{
    ...
    i2c_data_read(0x81, 1, version_info, 3);
 ...
 * 进行固件升级
 */
XBH_S32 ChipLt8711UXE1::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    if (mState == LT8711_RUNNING)
    {
...
XBH_S32 ChipLt8711UXE1::getChipExist(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XLOGE("%s %d \n",__func__,__LINE__);
    ...
    if(chip_id[0] != 0x21 ||  chip_id[1] != 0x01)
    {
...
XBH_S32 ChipLt8711UXE1::getChipInitDone(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}

XBH_S32 ChipLt8711UXE1::i2c_data_read(unsigned int u32RegAddr, unsigned int RegAddrLen, unsigned char *p_data, unsigned int data_len)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 ChipLt8711UXE1::i2c_data_write(unsigned char u8RegAddr, unsigned char reg_value)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 ChipLt8711UXE1::i2c_data_write_nbytes(unsigned int u32RegAddr, unsigned int RegAddrLen, unsigned char *p_data, unsigned int data_len)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 ChipLt8711UXE1::lt8711_i2c_write_data_to_flash(int Addr, unsigned char data)
{
    XLOGE("\n===> %s \n",__FUNCTION__);
...
XBH_S32 ChipLt8711UXE1::lt8711_i2c_read_data_from_flash(int Addr)
{
    XLOGE("\n===> %s \n",__FUNCTION__);
	int addr1,addr2,addr3;
...
//逐个字节比较
XBH_S32 ChipLt8711UXE1::lt8711_read_byte_compare(unsigned int u32RegAddr, unsigned int RegAddrLen, unsigned char *p_data, unsigned int data_len)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 ChipLt8711UXE1::open_upgrade_file(const char *upgrade_file_path)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XLOGD("\n===> %s\n",__FUNCTION__);
...
XBH_S32 ChipLt8711UXE1::lt8711_load_code_to_flash(int Addr)
{
    XLOGD("\n===> %s\n",__FUNCTION__);
    XBH_S32 s32Ret = XBH_SUCCESS;
    unsigned char data;
    lt8711_i2c_to_flash_config();   //写入配置
    lt8711_flash_block_erase(Addr);     //擦除flash
    lt8711_download_core(Addr);         //下载代码
    usleep(10*1000);

    if(XBH_SUCCESS != lt8711_read_flash(Addr)) {
        return XBH_FAILURE;
    }
    lt8711_i2c_write_data_to_flash(Flash_Addr_CRC,g_crc_val);
    data = lt8711_i2c_read_data_from_flash(Flash_Addr_CRC);
    XLOGD("==>lt8711_loadCustomerCode_to_flash data =0x%02x\r\n",data);
    if(g_crc_val == data){
...
XBH_S32 ChipLt8711UXE1::lt8711_flash_block_erase(int Addr)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XLOGD("\n===> %s\n",__FUNCTION__);
    unsigned char addr1,addr2,addr3;
    g_lt8711_flash_addr=Addr;
...
XBH_S32 ChipLt8711UXE1::lt8711_i2c_to_flash_config(void)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XLOGD("\n===> %s\n",__FUNCTION__);
    //configure parameter
...
XBH_S32 ChipLt8711UXE1::lt8711_download_core(int Addr)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XLOGD("\n===> %s\n",__FUNCTION__);
    unsigned char addr1,addr2,addr3;
    ...
    //i2c_data_write(0xFF,0xE0);
    //i2c_data_write(0xEE,0x01);
    while((g_lt8711_count+32)<g_data_buffer_len)
    {
        addr1=g_lt8711_flash_addr;
        addr2=g_lt8711_flash_addr>>8;
        addr3=g_lt8711_flash_addr>>16;
        ...
        i2c_data_write( 0x5A , 0x00 );
        i2c_data_write( 0x58 , 0x21 );
        s32Ret = i2c_data_write_nbytes(0x59, 1, g_data_buffer+g_lt8711_count,32);
        i2c_data_write( 0x5B , addr3 );//addr
        i2c_data_write( 0x5C , addr2 );//addr
        i2c_data_write( 0x5D , addr1 );//addr
        i2c_data_write( 0x5A , 0x10 );
        i2c_data_write( 0x5A , 0x00 );
        g_lt8711_flash_addr=g_lt8711_flash_addr+0x20;
        g_lt8711_count=g_lt8711_count+32;
    }
    addr1=g_lt8711_flash_addr;
    addr2=g_lt8711_flash_addr>>8;
    addr3=g_lt8711_flash_addr>>16;
    ...
    i2c_data_write( 0x5A , 0x00 );
    i2c_data_write( 0x58 , 0x21 );
    s32Ret = i2c_data_write_nbytes(0x59, 1, g_data_buffer+g_lt8711_count, g_data_buffer_len-g_lt8711_count);
    i2c_data_write( 0x5B , addr3 );//addr
    i2c_data_write( 0x5C , addr2 );//addr
    i2c_data_write( 0x5D , addr1 );//addr
    i2c_data_write( 0x5A , 0x10 );
...
XBH_S32 ChipLt8711UXE1::lt8711_read_flash(int Addr)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XLOGD("\n===> %s\n",__FUNCTION__);
    unsigned char addr1,addr2,addr3;
    ...
    while((g_lt8711_count+32) < g_data_buffer_len)
    {
        ...
        i2c_data_write( 0x5A , 0x00 );
        i2c_data_write( 0x58 , 0x21 );
        s32Ret = lt8711_read_byte_compare(0x5f, 1, g_data_buffer+g_lt8711_count,32);
        g_lt8711_flash_addr = g_lt8711_flash_addr+0x20;
        g_lt8711_count = g_lt8711_count+32;
    }
    addr1=g_lt8711_flash_addr;
    ...
    i2c_data_write( 0x5A , 0x00 );
    i2c_data_write( 0x58 , 0x21 );
    s32Ret = lt8711_read_byte_compare(0x5f, 1, g_data_buffer+g_lt8711_count, g_data_buffer_len-g_lt8711_count);
...
XBH_S32 ChipLt8711UXE1::lt8711_read_key()
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}

XBH_S32 ChipLt8711UXE1::lt8711_download_key(unsigned char *Data)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}
...
//比较key
XBH_S32 ChipLt8711UXE1::lt8711_read_key_compare(unsigned char *Data)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}

XBH_S32 ChipLt8711UXE1::checkVersion()
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    unsigned char version_info[3] = {0};
    i2c_data_write(0xff, 0xe0);
...
/**
    根据OPS插入状态，动态设置PD功率
*/
XBH_S32 ChipLt8711UXE1::setChargingPower(XBH_PDCAPACITY_POWER data)
{
    XBH_S32 ret = XBH_SUCCESS;
    XBH_S32 chargingLevel = 0x00;
    switch (data)
    {
    ...
    XLOGD("LT8711UXE1 Charging Power Level is %d, 15W = 2, 65W = 0, 100W = 1\n", chargingLevel);
    i2c_data_write(0xFF, LT8711_REG_CHARGING_POWER >> 8);   //写入高8位地址
    i2c_data_write(LT8711_REG_CHARGING_POWER & 0xFF, chargingLevel); // 写入数据
 ...
 *******************************************************************************
 */
unsigned int ChipLt8711UXE1::bitsreverse(unsigned int inVal, unsigned char bits)
{
 ...
 *******************************************************************************
 */
unsigned int ChipLt8711UXE1::getcrc(CrcInfoTypeS type, unsigned char *buf, unsigned int bufLen)
{
...
unsigned char ChipLt8711UXE1::lt8711uxc_get_crc(unsigned char *upgradeData, XBH_U32 len)
{
    CrcInfoTypeS type =
    {
        .Width = 8,
...
void ChipLt8711UXE1::run(const void* arg)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    ...
    //允许升级
    if(XBH_SUCCESS != lt8711_load_code_to_flash(FW_FLASH_ADDR) || XBH_SUCCESS != lt8711_load_code_to_flash(FW2_FLASH_ADDR)){ //升级固件
...
ChipLt8711UXE1::ChipLt8711UXE1(XBH_U32 i2cBus, XBH_U32 Addr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel)
{
    XLOGD(" begin ");
    mState = LT8711_IDLE;
