#ifndef XBH_CHIP_LT8711UXE1_H
#define XBH_CHIP_LT8711UXE1_H
...
#define LT8711_REG_CHARGING_POWER  (0xE0B2)
...
class ChipLt8711UXE1 : public XbhMWThread, public XbhUsbc2HdmiInterface
{
...
public:
     ...
     * rLevel       复位工作时的电平状态
    */
    ChipLt8711UXE1(XBH_U32 i2cBus, XBH_U32 Addr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel);
    ~ChipLt8711UXE1();
    /**
     * 复位当前转换芯片
     */
    ...
    */
     ...
     */
    XBH_S32 setTypecCapacity(XBH_U32 value);

    /**
     根据OPS插入状态，动态设置PD功率
     */
    XBH_S32 setChargingPower(XBH_PDCAPACITY_POWER data);
...
private:
    ...
    XBH_S32 lt8711_i2c_write_data_to_flash(int Addr,unsigned char data);
    XBH_S32 lt8711_i2c_read_data_from_flash(int Addr);
    //command
    XBH_S32 open_upgrade_file(const char *upgrade_file_path);
    XBH_S32 lt8711_i2c_to_flash_config(void);
    XBH_S32 lt8711_read_key();
    XBH_S32 lt8711_flash_block_erase(int Addr);
    XBH_S32 lt8711_download_core(int Addr);
    ...
    XBH_S32 lt8711_read_flash(int Addr);
    XBH_S32 lt8711_load_code_to_flash(int Addr);

    XBH_S32 checkVersion();

    unsigned int bitsreverse(unsigned int inVal, unsigned char bits);
    unsigned int getcrc(CrcInfoTypeS type, unsigned char *buf, unsigned int bufLen);
    unsigned char lt8711uxc_get_crc(unsigned char *upgradeData, unsigned int len);
