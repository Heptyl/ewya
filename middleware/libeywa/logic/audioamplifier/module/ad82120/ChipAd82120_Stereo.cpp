#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@ChipAd82120_Stereo"
#include "XbhLog.h"
...
XbhMutex                 ChipAd82120_Stereo::mLock;
 ...
 * 复位功放芯片
*/
XBH_S32 ChipAd82120_Stereo::resetChip()
{
 ...
 * 设置speaker 软mute的状态
*/
XBH_S32 ChipAd82120_Stereo::setMute(XBH_BOOL mute)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_U8 data[1] = {0};
 ...
 * 获取speaker mute的状态
*/
XBH_S32 ChipAd82120_Stereo::getMute(XBH_BOOL* mute)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_U8 data[1] = {0};
 ...
 * 设置功放输出的增益值
*/
XBH_S32 ChipAd82120_Stereo::setGain(XBH_S32 gain)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 获取功放输出的增益值
*/
XBH_S32 ChipAd82120_Stereo::getGain(XBH_S32* gain)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 获取amp是否初始化完成
*/
XBH_S32 ChipAd82120_Stereo::getChipInitDone(XBH_BOOL* enable)
{
    if(m_bInitDone)
    {
        *enable = XBH_TRUE;
 ...
 * 获取chip是否存在
*/
XBH_S32 ChipAd82120_Stereo::getChipExist(XBH_BOOL* bEnable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}
 ...
 * 获取固件版本
 */
XBH_S32 ChipAd82120_Stereo::getFirmwareVersion(XBH_CHAR* strVersion)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 进行固件升级
 */
XBH_S32 ChipAd82120_Stereo::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 获取固件升级时的状态
 */
XBH_S32 ChipAd82120_Stereo::getUpgradeState(XBH_S32 * s32State)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}

XBH_S32 ChipAd82120_Stereo::Ad82120_Write(XBH_U8 *buf, XBH_U8 len)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}

XBH_S32 ChipAd82120_Stereo::Ad82120_Read(XBH_U8 *buf, XBH_U8 ctrl)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return s32Ret;
}

XBH_S32 ChipAd82120_Stereo::writeRegMap(ampRegMap *reg_map, XBH_S32 iicBus, XBH_S32 iicAddr)
{
...
XBH_S32 ChipAd82120_Stereo::writeEqRegMap(ampRegMap *reg_map, int *eq_reg, int value, XBH_S32 iicBus, XBH_S32 iicAddr)
{
    XBH_S32 ret = XBH_FAILURE;
    XBH_U8 addr;
    XBH_U32 size;
...
XBH_S32 ChipAd82120_Stereo::ParseIniConfig(const char *section, char *fileBuf, ampRegMap *pRegMap, XBH_U32 maxRegSize)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 ChipAd82120_Stereo::LoadIniFileBuf(const XBH_CHAR *fileName, XBH_CHAR *buf, XBH_S32 *fileSize, XBH_S32 bufSize)
{
    ...
    in.read(buf, bufSize);
    close(file);
    XLOGE("LoadIniFile:open file :%s success\n", fileName);
    return XBH_SUCCESS;
}

XBH_S32 ChipAd82120_Stereo::WriteIniConfig(const XBH_CHAR *fileName)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    ...
    if(ParseIniConfig(AUDIO_INI_AD82120_INIT_REG, pRawBuffer, pAd82120InitReg, AMP_MAX_REG_NUM) == XBH_SUCCESS)
    {
    ...
    usleep(20*1000);
    memset(pAd82120InitReg, 0xff, sizeof(ampRegMap) * AMP_MAX_REG_NUM);
    if (ParseIniConfig(AUDIO_INI_AD82120_RAM1_TAB, pRawBuffer, pAd82120InitReg, AMP_MAX_REG_NUM) == XBH_SUCCESS)
    {
    ...
    usleep(20*1000);
    memset(pAd82120InitReg, 0xff, sizeof(ampRegMap) * AMP_MAX_REG_NUM);
    if (ParseIniConfig(AUDIO_INI_AD82120_RAM2_TAB, pRawBuffer, pAd82120InitReg, AMP_MAX_REG_NUM) == XBH_SUCCESS)
    {
...
XBH_S32 ChipAd82120_Stereo::setDirectPass(XBH_BOOL enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_S32 eq_length = 4;
...
XBH_S32 ChipAd82120_Stereo::getDirectPass(XBH_BOOL* enable)
{   
    XBH_S32 s32Ret = XBH_FAILURE;
    if(enable == NULL)
    {
...
XBH_S32 ChipAd82120_Stereo::init()
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_CHAR strProjectId[8];
    m_bInitDone = XBH_FALSE;
#if ENABLE_AUDIO_INI_CONFIG
    s32Ret  = XbhPartitionData::getInstance()->getProjectId(strProjectId);
    if (s32Ret == XBH_SUCCESS) 
    {  
        if(strcmp(strProjectId, "0") == 0)
        {
            XLOGD("getProjectId_0: %s, XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE\n", strProjectId);
            WriteIniConfig(XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE);
        }
        else if(strcmp(strProjectId, "1") == 0)
        {
            XLOGD("getProjectId_1: %s, XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE\n", strProjectId);
            WriteIniConfig(XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE);       
        }
        else if(strcmp(strProjectId, "2") == 0)
        {
            XLOGD("getProjectId_2: %s, XBH_AUDIO_AQ_STR_SIZE_55_INI_FILE\n", strProjectId);
            WriteIniConfig(XBH_AUDIO_AQ_STR_SIZE_55_INI_FILE);
        }
        else if(strcmp(strProjectId, "3") == 0)
        {
            XLOGD("getProjectId_3: %s, XBH_AUDIO_AQ_STR_SIZE_65_INI_FILE\n", strProjectId);
            WriteIniConfig(XBH_AUDIO_AQ_STR_SIZE_65_INI_FILE);
        }
        else
        {
            XLOGD("getProjectId_else: %s, XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE\n", strProjectId);
            WriteIniConfig(XBH_AUDIO_AQ_STR_SIZE_43_INI_FILE);         
        }
    } 
    else 
    {  
        XLOGD("Failed to get project ID, error: %d\n", s32Ret);  
    }
#endif
    WriteIniConfig(XBH_AUDIO_AQ_STR_INI_FILE);
    m_bInitDone = XBH_TRUE;
    return s32Ret;
}

ChipAd82120_Stereo::ChipAd82120_Stereo(XBH_S32 iicBus, XBH_S32 iicAddr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel, XBH_S32 mute_gpio, XBH_S32 mute_level)
{
    XLOGD(" begin ");
    mI2cBus = iicBus;
    mI2cAddr = iicAddr;
...
ChipAd82120_Stereo::~ChipAd82120_Stereo()
{
