#ifndef __XBH_AUDIO_AMPLIFIER_TASK_H__
#define __XBH_AUDIO_AMPLIFIER_TASK_H__
...
class XbhAudioAmplifierTask : public XbhMWThread
{
public:
    static XbhAudioAmplifierTask* getInstance();
    /**
     * 复位功放芯片
    */
    XBH_S32 resetChip(XBH_AUDIO_CHANNEL_E channel);
    /**
     * 设置speaker mute的状态
    */
    XBH_S32 setMute(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL mute);
    /**
     * 获取speaker mute的状态
    */
    XBH_S32 getMute(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL* mute);
    /**
     * 设置功放输出的增益值
    */
    XBH_S32 setGain(XBH_AUDIO_CHANNEL_E channel, XBH_S32 gain);
    /**
     * 获取功放输出的增益值
    */
    XBH_S32 getGain(XBH_AUDIO_CHANNEL_E channel, XBH_S32* gain);
    /**
     * 设置输出的音频频率直通，不对频率做抑制，主要是针对subwoofer
     * 工厂测试的音频是1K HZ，subwoofer抑制了高频信号，导致无法测试到声音输出
    */
    XBH_S32 setDirectPass(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL enable);
    /**
     * 获取输出的音频频率直通的状态
    */
    XBH_S32 getDirectPass(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL* enable);
    /**
     * 获取amp是否初始化完成
    */
    XBH_S32 getChipInitDone(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL* enable);
    /**
     * 获取chip是否存在
    */
    XBH_S32 getChipExist(XBH_AUDIO_CHANNEL_E channel, XBH_BOOL* bEnable);
    /**
     * 获取固件版本
     */
    ...
    */
    ...
    ~XbhAudioAmplifierTask();

private:
    void run(const void* arg);
    XbhAudioAmplifierTask();
    static XbhAudioAmplifierTask *mInstance;
    static XbhMutex mLock;

    XbhAudioAmplifierInterface *m_pXbhAudioAmplifierInterface[3];

    XBH_U8 mList1[2];
    AMP_OPT_TABLE mList2[2];
};
