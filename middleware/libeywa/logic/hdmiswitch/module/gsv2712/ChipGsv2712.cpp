#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@ChipGsv2712"
...
XbhMutex                        ChipGsv2712::mLock;

ChipGsv2712::ChipGsv2712(XBH_S32 i2cNumber, XBH_S32 i2cAddress, XBH_S32 powerGpio, XBH_S32 powerLevel ,XBH_S32 resetGpio, XBH_S32 resetLevel , XBH_S32 defaultPort , HDMI_SWITCH_PORT u8SwitchPort, XBH_U8 level)
{
...
ChipGsv2712::~ChipGsv2712()
{
    XLOGD("~ end");
}

XBH_S32 ChipGsv2712::setPowerChip(XBH_BOOL onoff)
{
...
XBH_S32 ChipGsv2712::setActivePort(XBH_U8 port)
{
    ...
    {
        case 0:
            data = GSV2712_PORT_RX_A;
            break;
        case 1:
            data = GSV2712_PORT_RX_B;;
            break;
        case 2:
            data = GSV2712_PORT_RX_C;
            break;
        case 3:
            data = GSV2712_PORT_RX_D;
            break;
        default:
            XLOGE("Error port\n");
            break;
...
XBH_S32 ChipGsv2712::getActivePort(XBH_U8* port)
{
...
XBH_S32 ChipGsv2712::getRxPortStatus(XBH_U8 port, XBH_U8* status)
{
    XbhMutex::Autolock _l(mLock);
    XBH_U8 data;
    * status = 0;
...
XBH_S32 ChipGsv2712::getTxPortStatus(XBH_U8* status)
{
    XbhMutex::Autolock _l(mLock);
    XBH_U8 data;
...
XBH_S32 ChipGsv2712::setRxHpd(XBH_U8 port, XBH_BOOL high)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_U8 origin = 0x00;
...
XBH_S32 ChipGsv2712::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    XbhMutex::Autolock _l(mLock);
    XLOGD("upgradeFirmware  strFilePath %s",strFilePath);
    property_set(GSV2712_VERSION, "000000");
    forceUpgrade = true;
    mDevType = 0x2712;
...
XBH_S32 ChipGsv2712::getFirmwareVersion(XBH_CHAR* strVersion)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 ChipGsv2712::resetChip(XBH_BOOL hw)
{
    XbhMutex::Autolock _l(mLock);
...
XBH_S32 ChipGsv2712::setHPD(XBH_U8 portId, XBH_BOOL bHigh)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_U8 data = 0x00;
...
XBH_U8 ChipGsv2712::calcCRC8(const XBH_U8 *data, XBH_U32 len)
{
    XBH_U8 crc8 = 0;
    XBH_U16 i = 0;
    while (len --)
    {
        crc8 = crc8 ^ data[i];
        crc8 = crc8Gsv2712Table[crc8];
        i++;
    }
    return crc8;
}

XBH_S32 ChipGsv2712::setChargingPower(XBH_U8 data)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 ret = XBH_SUCCESS;
    switch (data)
    {
        case XBH_PDCAPACITY_0W: // 0W
            data = 0x00;
            break;
        case XBH_PDCAPACITY_15W:// 15w
            data = 0x01;
            break;
        case XBH_PDCAPACITY_45W:// 45w
            data = 0x02;
            break;
        ...
        case XBH_PDCAPACITY_100W:// 100w
            data = 0x04;
            break;
        case XBH_PDCAPACITY_4_5W:// 4.5w
            data = 0x05;
            break;
        default:
            break;
    }
    XLOGD("GSV2712 Charging Power Level is %d, 15W = 1, 45W = 2, 65W = 3, 100W = 4 4.5W = 5\n", data);
    ret = XbhService::getModuleInterface()->setI2cData(mI2cNumber, mI2cAddress,REG_CHARGING_POWER,2,1,&data);
    return ret;
}

XBH_S32 ChipGsv2712::writeData(XBH_S32 reg_addr, XBH_U8 *p_data, XBH_U32 data_lenth)
{
    XBH_S32 ret = XBH_FAILURE;
    ...
    {
        ret = XbhService::getModuleInterface()->setI2cData(mI2cNumber, XBH_GSV2712_DEVICE_ADDRESS, reg_addr, 2, data_lenth, p_data);
        if (ret != XBH_SUCCESS)
        {
...
XBH_S32 ChipGsv2712::readData(XBH_S32 reg_addr, XBH_U8 *p_data, XBH_U32 data_lenth)
{
    XBH_S32 ret = XBH_FAILURE;
...
XBH_S32 ChipGsv2712::getUpgradeState(XBH_S32 *pstat)
{
    *pstat = upgradeState;
    XLOGD("[%s:%d] status:%d", __func__, __LINE__, upgradeState);
    return (upgradeState == GSV2712Upgrade_SUCCESS) ? XBH_SUCCESS : XBH_FAILURE;
}

XBH_S32 ChipGsv2712::setPhysicalAddr(XBH_U8 port, XBH_U16 cecAddr)
{
    XbhMutex::Autolock _l(mLock);
    XLOGD("port = %d",port);
...
XBH_S32 ChipGsv2712::writeRXDDCUpdConfigID(void)
{
    XBH_U16 i;
    XBH_U8 tmp[4];
    //write
    writeData(XBH_GSV2712_RXDDC_UPDCONFIGID, (XBH_U8 *)UpdConfigID, 4);
    //wait reply
    for(i = 0; i < 100; i++)
    {
        usleep (20 * 1000);
        readData(XBH_GSV2712_RXDDC_UPDREPLYID, tmp, 4);
        ...
        {
            usleep (20 * 1000);
            readData(XBH_GSV2712_RXDDC_UPDCONFIGID, tmp, 4);
...
XBH_S8 ChipGsv2712::writeRXDDCUpdInfo(XBH_U32 appSize, XBH_U32 appAddr)
{
    GSV2712UpgradeInfo Info;
    Info.DataTotalLen = appSize;
    Info.DataOffsetAtFlash = appAddr;
    Info.VersionMajor = XBH_GSV2712_APP_VERSION_MAJOR;
    Info.VersionMinor = XBH_GSV2712_APP_VERSION_MINOR;
    Info.GsvChipNumber = mDevType;
    //checksum
    XBH_U8 dataSum = calcCRC8((XBH_U8 *)&Info, sizeof(GSV2712UpgradeInfo));
    //write
    writeData(XBH_GSV2712_RXDDC_UPDINFO, (XBH_U8 *)&Info, sizeof(GSV2712UpgradeInfo));
    writeData(XBH_GSV2712_RXDDC_INFOCHECKSUM, &dataSum, 1);
    //
    return 0;
}

XBH_S8 ChipGsv2712::writeRXDDCUpdData(XBH_U8 *pbuf)
{
    XBH_U8 dataSum = 0;
    dataSum = calcCRC8(pbuf, XBH_TRANSFER_DATA_BLKSIZE);
    writeData(XBH_GSV2712_RXDDC_UPDDATA, pbuf, XBH_TRANSFER_DATA_BLKSIZE);
    writeData(XBH_GSV2712_RXDDC_DATACHECKSUM, &dataSum, 1);
    //
    return 0;
}

 XBH_S8 ChipGsv2712::writeMsgCmd(void)
{
    writeData(XBH_GSV2712_RXDDC_COMMAND, &RXDDCCmd, 1);
    return 0;
}

XBH_S8 ChipGsv2712::waitMsgAckNew(void)
{
    XBH_U16 i;
    XBH_U8 msgAck = 0;
    for(i = 0; i < 5000; i++) {
        usleep (1000);
        readData(XBH_GSV2712_RXDDC_REPLY, &msgAck, 1);
...
XBH_S8 ChipGsv2712::msgCmdAndWaitNew(void)
{
    writeData(XBH_GSV2712_RXDDC_COMMAND, &RXDDCCmd, 1);
    if(waitMsgAckNew() < 0)
        return -1;
    else {
        RXDDCCmd += XBH_GSV2712_ROLL_BACK_INCDATA;
        return 0;
    }
}

XBH_S8 ChipGsv2712::appI2CUpdateInit(void)
{
    RXDDCCmd = 0;
    // writeData(XBH_GSV2712_RXDDC_REBOOT, &RXDDCCmd, 1);	//need clear
    XbhService::getModuleInterface()->setI2cData(mI2cNumber, XBH_GSV2712_DEVICE_ADDRESS,XBH_GSV2712_RXDDC_REBOOT, 2, 1,&RXDDCCmd);
...
XBH_S8 ChipGsv2712::appI2CUpdateServer(XBH_U8 * appData, XBH_U32 appSize, XBH_U32 appAddr)
{
    XBH_U32 i = 0;
app_start:
    if (i >= 3) {
    ...
    //fisrt step
    RXDDCCmd = XBH_GSV2712_RXDDC_INFO_CMD;
    ...
    while(1)
    {
        ...
        if(RXDDCDataPoint >= appSize)
        {
            ...
            //End
            RXDDCCmd = XBH_GSV2712_TRANSFER_END;
            writeData(XBH_GSV2712_RXDDC_UPDCONFIGID, &RXDDCCmd, 1); //write other value to exit loop
            writeData(XBH_GSV2712_RXDDC_REBOOT, &RXDDCCmd, 1);
            break;
        } 
        else
        {
            ...
            //next
            RXDDCCmd &= 0xf0;
            RXDDCCmd += XBH_GSV2712_RXDDC_DATA_CMD;
            if(msgCmdAndWaitNew() < 0)
            {
...
*/
XBH_S32 ChipGsv2712::powerUpProtection()
{
    XBH_S32 ret = XBH_SUCCESS;
    XBH_U8 loop = 0;
    XBH_U8 temp = 0;
...
XBH_S32 ChipGsv2712::setSpecificMode(void)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 ret = XBH_SUCCESS;
    ...
    {
        XLOGD("gsv2712 success enter standby!!! %x\n",standby_flag);
    }
    else
    {
        XLOGD("gsv2712 enter standby fail!!! %x\n",standby_flag);
        ret = XBH_FAILURE;
    }
    return ret;
}
...
XBH_S32 ChipGsv2712::getHdmiSwitchHdcpKey(XBH_U32 devType,XBH_CHAR* hdcpValue,XBH_HDCP_TYPE_E hdcpType)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_S32 ret = XBH_FAILURE;
    if(hdcpType == XBH_HDCP2_2)
    {
        ...
        {
            XLOGE("ChipGsv2712 getHdcpKeyId22 FAIL! ");
            return XBH_FAILURE;
        }
        else
        {
    ...
    else
    {
        ...
        {
            XLOGE("ChipGsv2712 getHdcpKeyId14 FAIL! ");
            return XBH_FAILURE;
        }else
        {
            //读取1.4 HDCP
...
XBH_S32 ChipGsv2712::setTypecReset()
{
    XLOGD("setTypecReset! \n");
...
void ChipGsv2712::run(const void* arg)
{
    ...
    if ((partition_data[0x8e]  != dataGsv2712Buff[0]) || (partition_data[0x8f]  != dataGsv2712Buff[1]) || forceUpgrade)
    {
        upgradeState = GSV2712Upgrade_RUNNING;
        if (appI2CUpdateServer(partition_data, 252 * 1024, patition_start_addr))
        {
            upgradeState = GSV2712Upgrade_SUCCESS;
            usleep(2*1000*1000);
            reset();
            usleep(2*1000*1000);
