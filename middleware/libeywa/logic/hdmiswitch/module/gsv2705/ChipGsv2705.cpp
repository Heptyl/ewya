#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@ChipGsv2705"
...
ChipGsv2705::~ChipGsv2705()
{
    XLOGD("~ end");
}


XBH_S32 ChipGsv2705::setPowerChip(XBH_BOOL onoff)
{
...
XBH_S32 ChipGsv2705::setActivePort(XBH_U8 port)
{
    ...
    {
        case 0:
            data = GSV2705_PORT_RX_A;
            break;
        case 1:
            data = GSV2705_PORT_RX_B;;
            break;
        case 2:
            data = GSV2705_PORT_RX_C;
            break;
        case 3:
            data = GSV2705_PORT_RX_D;
            break;
        default:
            XLOGE("Error port\n");
            break;
...
XBH_S32 ChipGsv2705::setPhysicalAddr(XBH_U8 port, XBH_U16 cecAddr)
{
    XbhMutex::Autolock _l(mLock);
    XLOGD("port = %d",port);
...
XBH_S32 ChipGsv2705::getActivePort(XBH_U8* port)
{
...
XBH_S32 ChipGsv2705::getRxPortStatus(XBH_U8 port, XBH_U8* status)
{
    XbhMutex::Autolock _l(mLock);
    XBH_U8 data;
    * status = 0;
...
XBH_S32 ChipGsv2705::getTxPortStatus(XBH_U8* status)
{
    XbhMutex::Autolock _l(mLock);
    XBH_U8 data;
...
XBH_S32 ChipGsv2705::setRxHpd(XBH_U8 port, XBH_BOOL high)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_U8 origin = 0x00;
...
XBH_S32 ChipGsv2705::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    XbhMutex::Autolock _l(mLock);
    XLOGD("upgradeFirmware  strFilePath %s",strFilePath);
    property_set(GSV2705_VERSION, "000000");
    forceUpgrade = true;
    mDevType = 0x2705;
...
XBH_S32 ChipGsv2705::getFirmwareVersion(XBH_CHAR* strVersion)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 ChipGsv2705::resetChip(XBH_BOOL hw)
{
    XbhMutex::Autolock _l(mLock);
...
XBH_U8 ChipGsv2705::calcCRC8(const XBH_U8 *data, XBH_U32 len)
{
    XBH_U8 crc8 = 0;
    XBH_U16 i = 0;
    while (len --)
    {
        crc8 = crc8 ^ data[i];
        crc8 = crc8Gsv2705Table[crc8];
        i++;
    }
    return crc8;
}

XBH_S32 ChipGsv2705::writeData(XBH_S32 reg_addr, XBH_U8 *p_data, XBH_U32 data_lenth)
{
    ...
    {
        ret = XbhService::getModuleInterface()->setI2cData(mI2cNumber, XBH_GSV2705_DEVICE_ADDRESS, reg_addr, 2, data_lenth, p_data);
        if (ret != XBH_SUCCESS)
        {
...
XBH_S32 ChipGsv2705::readData(XBH_S32 reg_addr, XBH_U8 *p_data, XBH_U32 data_lenth)
{
    XBH_S32 ret = XBH_FAILURE;
...
XBH_S32 ChipGsv2705::getUpgradeState(XBH_S32 *pstat)
{
    *pstat = upgradeState;
    XLOGD("[%s:%d] status:%d", __func__, __LINE__, upgradeState);
    return (upgradeState == GSV2705Upgrade_SUCCESS) ? XBH_SUCCESS : XBH_FAILURE;
}

void ChipGsv2705::reset(void)
{
    XbhService::getModuleInterface()->setGpioOutputValue(mResetGpio, mResetLevel);
    usleep(50*1000);
    XbhService::getModuleInterface()->setGpioOutputValue(mResetGpio, !mResetLevel);
...
XBH_S32 ChipGsv2705::writeRXDDCUpdConfigID(void)
{
    XBH_U16 i;
    XBH_U8 tmp[4];
    //write
    writeData(XBH_GSV2705_RXDDC_UPDCONFIGID, (XBH_U8 *)UpdConfigID, 4);
    //wait reply
    for(i = 0; i < 100; i++)
    {
        usleep (20 * 1000);
        readData(XBH_GSV2705_RXDDC_UPDREPLYID, tmp, 4);
        ...
        {
            usleep (20 * 1000);
            readData(XBH_GSV2705_RXDDC_UPDCONFIGID, tmp, 4);
...
XBH_S8 ChipGsv2705::writeRXDDCUpdInfo(XBH_U32 appSize, XBH_U32 appAddr)
{
    GSV2705UpgradeInfo Info;
    Info.DataTotalLen = appSize;
    Info.DataOffsetAtFlash = appAddr;
    Info.VersionMajor = XBH_GSV2705_APP_VERSION_MAJOR;
    Info.VersionMinor = XBH_GSV2705_APP_VERSION_MINOR;
    Info.GsvChipNumber = mDevType;
    //checksum
    XBH_U8 dataSum = calcCRC8((XBH_U8 *)&Info, sizeof(GSV2705UpgradeInfo));
    //write
    writeData(XBH_GSV2705_RXDDC_UPDINFO, (XBH_U8 *)&Info, sizeof(GSV2705UpgradeInfo));
    writeData(XBH_GSV2705_RXDDC_INFOCHECKSUM, &dataSum, 1);
    //
    return 0;
}

XBH_S8 ChipGsv2705::writeRXDDCUpdData(XBH_U8 *pbuf)
{
    XBH_U8 dataSum = 0;
    dataSum = calcCRC8(pbuf, XBH_TRANSFER_DATA_BLKSIZE);
    writeData(XBH_GSV2705_RXDDC_UPDDATA, pbuf, XBH_TRANSFER_DATA_BLKSIZE);
    writeData(XBH_GSV2705_RXDDC_DATACHECKSUM, &dataSum, 1);
    //
    return 0;
}

 XBH_S8 ChipGsv2705::writeMsgCmd(void)
{
    writeData(XBH_GSV2705_RXDDC_COMMAND, &RXDDCCmd, 1);
    return 0;
}

XBH_S8 ChipGsv2705::waitMsgAckNew(void)
{
    XBH_U16 i;
    XBH_U8 msgAck = 0;
    for(i = 0; i < 5000; i++) {
        usleep (1000);
        readData(XBH_GSV2705_RXDDC_REPLY, &msgAck, 1);
...
XBH_S8 ChipGsv2705::msgCmdAndWaitNew(void)
{
    writeData(XBH_GSV2705_RXDDC_COMMAND, &RXDDCCmd, 1);
    if(waitMsgAckNew() < 0)
        return -1;
    else {
        RXDDCCmd += XBH_GSV2705_ROLL_BACK_INCDATA;
        return 0;
    }
}

XBH_S8 ChipGsv2705::appI2CUpdateInit(void)
{
    RXDDCCmd = 0;
    // writeData(XBH_GSV2705_RXDDC_REBOOT, &RXDDCCmd, 1);	//need clear
    XbhService::getModuleInterface()->setI2cData(mI2cNumber, XBH_GSV2705_DEVICE_ADDRESS,XBH_GSV2705_RXDDC_REBOOT, 2, 1,&RXDDCCmd);
...
XBH_S8 ChipGsv2705::appI2CUpdateServer(XBH_U8 * appData, XBH_U32 appSize, XBH_U32 appAddr)
{
    XBH_U32 i = 0;
app_start:
    if (i >= 3) {
    ...
    //fisrt step
    RXDDCCmd = XBH_GSV2705_RXDDC_INFO_CMD;
    ...
    while(1)
    {
        ...
        if(RXDDCDataPoint >= appSize)
        {
            ...
            //End
            RXDDCCmd = XBH_GSV2705_TRANSFER_END;
            writeData(XBH_GSV2705_RXDDC_UPDCONFIGID, &RXDDCCmd, 1); //write other value to exit loop
            writeData(XBH_GSV2705_RXDDC_REBOOT, &RXDDCCmd, 1);
            break;
        } 
        else
        {
            ...
            //next
            RXDDCCmd &= 0xf0;
            RXDDCCmd += XBH_GSV2705_RXDDC_DATA_CMD;
            if(msgCmdAndWaitNew() < 0)
            {
...
*/
XBH_S32 ChipGsv2705::powerUpProtection()
{
...
XBH_S32 ChipGsv2705::updateEdid(XBH_U8 edid_data[256],XBH_U8 port)
{
    XbhMutex::Autolock _l(mLock);
    XBH_S32 ret = XBH_SUCCESS;
    XBH_U8 data = 0xa0;
...
void ChipGsv2705::run(const void* arg)  {
    ...
    else if(mWdtRunning == GSV2705_WDT_RUNNING)
    {
        ...
        if (readData(0xFF04, first_value, 2) != XBH_SUCCESS) {
            XLOGE("Failed to read first value");
            return;
        }
        ...
        for (int i = 1; i < 10; i++) {
            usleep(100 * 1000);
            if (readData(0xFF04, current_value, 2) != XBH_SUCCESS) {
                XLOGE("Failed to read value %d", i);
                all_same = false;
                break;
            }
            // 比较当前值与第一次值
            if (memcmp(current_value, first_value, 2) != 0) {
                all_same = false;
                break;
            }
        }
        ...
        if (all_same) {
            XLOGW("################################################");
            XLOGW("#[WTD] CRASH!!! reset RST and setActivePort :%d#", mCurrentPort);
            XLOGW("###############################################");
            XbhService::getModuleInterface()->setGpioOutputValue(XBH_BOARD_GPIO_GVS2705_HDMI_SW_RST, !XBH_BOARD_GPIO_GVS2705_HDMI_SW_RST_ON);
            usleep(200 * 1000);
            XbhService::getModuleInterface()->setGpioOutputValue(XBH_BOARD_GPIO_GVS2705_HDMI_SW_RST, XBH_BOARD_GPIO_GVS2705_HDMI_SW_RST_ON);
            mWdtResetCount++;
            usleep(100 * 1000);
            setActivePort(mCurrentPort);
        } else {
            XLOGV("[WTD] Feeding, reset count : %d", mWdtResetCount);
            usleep(3*1000 * 1000);
        }
    }
