#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@XbhTouchYclUart"
...
XbhTouchYclUart*         XbhTouchYclUart::mInstance = XBH_NULL;
XbhMutex                    XbhTouchYclUart::mLockObject;

/**
 * 设置信号源的触摸开关。
 * enable 开关状态
 */
XBH_S32 XbhTouchYclUart::setSourceTouchState(XBH_BOOL enable)
{
    XLOGE("setSourceTouchState enable = %d", enable);
 ...
 * boolean 开关状态
 */
XBH_S32 XbhTouchYclUart::getSourceTouchState(XBH_BOOL *enable)
{
    XLOGE("getSourceTouchState");
    XBH_S32 ret = XBH_FAILURE;
 ...
 * ht = (h*32767)/2160
 */
XBH_S32 XbhTouchYclUart::setNonThroughTouchRegion(XBH_S32 id, XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XLOGE("setNonThroughTouchRegion %d %d (%04d, %04d) (%04d, %04d)", id, resolution, x, y, w, h);
    XBH_S32 ret = XBH_FAILURE;
 ...
 * id: 当前区域的id
 */
XBH_S32 XbhTouchYclUart::deleteNonThroughTouchRegion(XBH_S32 id, XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XLOGE("deleteNonThroughTouchRegion %d %d (%04d, %04d) (%04d, %04d)", id, resolution, x, y, w, h);
    XBH_S32 ret = XBH_FAILURE;
...
*/
XBH_S32 XbhTouchYclUart::setTouchScaleRect(XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XLOGE("setTouchScaleRect %d (%04d, %04d) (%04d, %04d)", resolution, x, y, w, h);
    XBH_S32 ret = XBH_FAILURE;
 ...
 * 发送按键给触摸框，由触摸框转发
 */
XBH_S32 XbhTouchYclUart::sendKeyToTp(XBH_U32 keyCode)
{
    XLOGE("sendKeyToTp keyCode = %d", keyCode);
    XBH_S32 ret = XBH_FAILURE;
    if(mSendKeyToTp)
    {
 ...
 * 设置触摸框防误触开关
 */ 
XBH_S32 XbhTouchYclUart::setPalmRejection(XBH_BOOL enable)
{
    XLOGE("setPalmRejection enable = %d", enable);
    XBH_S32 ret = XBH_FAILURE;
 ...
 *  获取触摸框防误触开关
 */ 
XBH_S32 XbhTouchYclUart::getPalmRejection(XBH_BOOL *enable)
{
    XLOGE("getPalmRejection");
    XBH_S32 ret = XBH_FAILURE;
...
XBH_S32 XbhTouchYclUart::Init(XBH_VOID)
{
    XBH_S32 s32BaudRate;
    ...
    mTouchHandle = dlopen("/vendor/lib64/libycluarttouch.so", RTLD_LAZY);
    if(!mTouchHandle)
    {
        XLOGE("open libycluarttouch.so fail.");
        return 0;
    }
...
XbhTouchYclUart::XbhTouchYclUart(XBH_CHAR *ko, XBH_CHAR *tpName)
{
...
XbhTouchYclUart::~XbhTouchYclUart()
{
    if(mInstance != NULL)
    {
        delete mInstance;
...
XbhTouchInterface *XbhTouchYclUart::getInstance(XBH_CHAR *ko, XBH_CHAR *tpName)
{
    XbhMutex::Autolock lock(mLockObject);
    if (mInstance == XBH_NULL) {
        mInstance = new XbhTouchYclUart(ko, tpName);
    }
    return mInstance;
}
