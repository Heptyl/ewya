#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@XbhTouchCommonUart"
...
XbhTouchCommonUart*         XbhTouchCommonUart::mInstance = XBH_NULL;
XbhMutex                    XbhTouchCommonUart::mLockObject;

/**
 * 设置信号源的触摸开关。
 * enable 开关状态
 */
XBH_S32 XbhTouchCommonUart::setSourceTouchState(XBH_BOOL enable)
{
 ...
 * boolean 开关状态
 */
XBH_S32 XbhTouchCommonUart::getSourceTouchState(XBH_BOOL *enable)
{
    XBH_S32 ret = XBH_FAILURE;
 ...
 * ht = (h*32767)/2160
 */
XBH_S32 XbhTouchCommonUart::setNonThroughTouchRegion(XBH_S32 id, XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XBH_S32 ret = XBH_FAILURE;
 ...
 * id: 当前区域的id
 */
XBH_S32 XbhTouchCommonUart::deleteNonThroughTouchRegion(XBH_S32 id, XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XBH_S32 ret = XBH_FAILURE;
...
*/
XBH_S32 XbhTouchCommonUart::setTouchScaleRect(XBH_S32 resolution, XBH_S32 x, XBH_S32 y, XBH_S32 w, XBH_S32 h)
{
    XBH_S32 ret = XBH_FAILURE;
 ...
 * 发送按键给触摸框，由触摸框转发
 */
XBH_S32 XbhTouchCommonUart::sendKeyToTp(XBH_U32 keyCode)
{
    XBH_S32 ret = XBH_FAILURE;
    if(mSendKeyToTp)
    {
 ...
 * 设置触摸框防误触开关
 */ 
XBH_S32 XbhTouchCommonUart::setPalmRejection(XBH_BOOL enable)
{
    XBH_S32 ret = XBH_FAILURE;
 ...
 *  获取触摸框防误触开关
 */ 
XBH_S32 XbhTouchCommonUart::getPalmRejection(XBH_BOOL *enable)
{
    XBH_S32 ret = XBH_FAILURE;
...
XBH_S32 XbhTouchCommonUart::Init(XBH_VOID)
{
    XBH_S32 s32BaudRate;
    ...
    mTouchHandle = dlopen("/vendor/lib64/libxbhuarttouch.so", RTLD_LAZY);
    if(!mTouchHandle)
    {
        XLOGE("open libxbhuarttouch.so fail.");
        return 0;
    }
...
XbhTouchCommonUart::XbhTouchCommonUart(XBH_CHAR *ko, XBH_CHAR *tpName)
{
...
XbhTouchCommonUart::~XbhTouchCommonUart()
{
    if(mInstance != NULL)
    {
        delete mInstance;
...
XbhTouchInterface *XbhTouchCommonUart::getInstance(XBH_CHAR *ko, XBH_CHAR *tpName)
{
    XbhMutex::Autolock lock(mLockObject);
    if (mInstance == XBH_NULL) {
        mInstance = new XbhTouchCommonUart(ko, tpName);
    }
    return mInstance;
}
