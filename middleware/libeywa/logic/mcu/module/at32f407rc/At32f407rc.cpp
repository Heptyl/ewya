#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@At32f407rc"
#define INIT  0xffffffff
#define MAX_SEND_FILE_SIZE      64
...
XBH_S32 At32f407rc::WriteMcuMultiBytes(XBH_U8 s32RegAddr, XBH_U32 u32Length,  XBH_U8 *pu8Data)
{
...
XBH_S32 At32f407rc::ReadMcuMultiBytes(XBH_U8 s32RegAddr, XBH_U32 u32Length, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 At32f407rc::resetChip()
{
    ...
    if (mRgpio > 0)
    {
        ...
        usleep(100 * 1000);         //at32f407 min reset time 20us
        s32Ret |= XbhService::getModuleInterface()->setGpioOutputValue(mRgpio, !mRLevel);
    }
    else
    {
 ...
 * 获取chip是否初始化完成
*/
XBH_S32 At32f407rc::getChipInitDone(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 获取chip是否存在
*/
XBH_S32 At32f407rc::getChipExist(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
...
//获取当前Mcu升级状态
XBH_S32 At32f407rc::getUpgradeState(XBH_S32* s32State)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    *s32State = mState;
...
XBH_S32 At32f407rc::WriteMcuByte(XBH_U8 s32RegAddr, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (!pu8Data)
    {
    ...
    s32Ret = XbhService::getModuleInterface()->setI2cData(mI2cBus, mI2cAddr, s32RegAddr, 1, 1, pu8Data);
    //XLOGD("writeI2cByte: s32Ret=%d,i2cNum=0x%x,s32RegAddr=0x%x,pu8Data=0x%x", s32Ret, (XBH_S32)Ep9179_I2C_NUM, s32RegAddr, pu8Data[0]);
    usleep(5);
    return s32Ret;
}

//2、mcu进入Iap模式
XBH_S32 At32f407rc::set_enter_iap_mode()
{
   XBH_S32 s32Ret = XBH_SUCCESS;

    XBH_U8  pu8Data[1] = {0x01};

    s32Ret = WriteMcuByte(CMD_I2C_ENTER_IAP, pu8Data);
    XLOGD("[%s]: WriteMcuByte: 0x%02x\n", __func__, CMD_I2C_ENTER_IAP);

    return s32Ret;
}

//3、写入mcu数据
XBH_S32 At32f407rc::upgrade_download_data(XBH_U8 *buff, XBH_U32 length)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 At32f407rc::calculate_CRC32_Mpeg2(void *pStart, unsigned int uiSize)
{
...
XBH_S32 At32f407rc::check_iap_CRC32(XBH_U32 pc_crc)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    ...
    if(s32Ret == XBH_SUCCESS)
    {
        ...
        XLOGD("app crc: %04x, mcu crc: %04x\n", pc_crc, mcu_crc);
        if((pc_crc == mcu_crc) || (mcu_crc == 0xffffffff))
        {
            return 1;
        }
    }
...
XBH_S32 At32f407rc::set_iap_status(XBH_U8 status)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    s32Ret = WriteMcuByte(CMD_I2C_WRITE_IAP_VALID, buf);

    return s32Ret;
}

//升级执行入口
XBH_S32 At32f407rc::MCUUpgrade(XBH_BOOL bForceUpgrade)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    XBH_U32 u32RetAppFileLength = 0;
    XBH_U8* pU8FileBuff = NULL;
    XBH_U8 strAppVersion[2];
    XBH_BOOL bIsUpdateHead = false;
    FILE* pFile = NULL;
    mState = AT32_RUNNING;
    ...
    XLOGD("---> FileCrc : 0x%08x", u32FileCrc);
    if (check_iap_CRC32(u32FileCrc) != true)
    {
...
XBH_S32 At32f407rc::mcuReadBytes(XBH_MCU_I2CBUFFDEF_S* pI2cBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
 ...
 * 获取MCU软件版本
 */
XBH_S32 At32f407rc::getMcuVersion(XBH_S32 *mcuVer)
{
    //XbhMutex::Autolock _l(mLock);
    ...
    stInData.len = 2;
    XLOGD("Func[%s] - Line[%d] getMcuVersion",__func__, __LINE__);
 ...
 * 获取升级固件版本
 */
XBH_S32 At32f407rc::getFirmwareVersion(XBH_CHAR* strVersion)
{
	XLOGD("At32f407rc : getFirmwareVersion !!!!!");
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_S32 mcuVer = 0;
 ...
 * 进行固件升级
 */
XBH_S32 At32f407rc::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
	XLOGD("At32f407rc : upgradeFirmware !!!!!");
    //XbhMutex::Autolock _l(mLock);
    ...
    mUpgradeStatus = XBH_UPGRADE_RUNNING;
...
XBH_S32 At32f407rc::setMcuFattMode(XBH_S32 mode)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 pu8Data[1];
...
XBH_S32 At32f407rc::getMcuFattMode(XBH_S32 *mode)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 buf[64] = {0x00};
...
XBH_S32 At32f407rc::setMcuIIcBypass(XBH_U8 u8IIcNum, XBH_U8 u8DeviceAddr, XBH_U8 u8RegAddr, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[4] = {0x00};
    param[0] = u8IIcNum;
    ...
    XLOGD("setMcuIIcBypass num:%d, addr:0x%02x, reg:0x%02x, len:0x%d, data[0]:%02x", u8IIcNum, u8DeviceAddr, u8RegAddr, u8Len, u8Data[0]);
    return s32Ret;
}

XBH_S32 At32f407rc::getMcuIIcBypass(XBH_U8 u8IIcNum, XBH_U8 u8DeviceAddr, XBH_U8 u8RegAddr, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[4] = {0x00};
    param[0] = u8IIcNum;
    ...
    XLOGD("getMcuIIcBypass num:%d, addr:0x%02x, reg:0x%02x, len:0x%d, data[0]:%02x", u8IIcNum, u8DeviceAddr, u8RegAddr, u8Len, u8Data[0]);
    return s32Ret;
}

XBH_S32 At32f407rc::setMcuUartBypassPara(XBH_U8 u8UartNum, XBH_U8 baud)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[2] = {0x00};
    param[0] = u8UartNum;
    ...
    XLOGD("setMcuUartBypassPara u8UartNum:%d, baud:0x%02x \n", u8UartNum, baud);
    return s32Ret;
}

XBH_S32 At32f407rc::setMcuUartBypass(XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    XLOGD("setMcuUartBypass len:%d\n", u8Len);
    return s32Ret;
}

XBH_S32 At32f407rc::getMcuUartBypass(XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    s32Ret = ReadMcuMultiBytes(CMD_I2C_GET_BYPASS_UART, u8Len, u8Data) ;
    XLOGD("getMcuUartBypass u8Len\n", u8Len);
    return s32Ret;
}

void At32f407rc::run(const void* arg)
{
    //XbhMutex::Autolock _l(mLock);
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (mUpgradeStatus == XBH_UPGRADE_RUNNING)
    {
...
At32f407rc::At32f407rc(XBH_S32 iicBus, XBH_S32 iicAddr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel)
{
    XLOGD(" begin ");
    mI2cBus = iicBus;
    mI2cAddr = iicAddr;
