#define LOG_NDEBUG 0
#define XBH_LOG_TAG "xbh_mw@Gd32f330r8"
...
XBH_S32 Gd32f330r8::WriteMcuMultiBytes(XBH_U8 s32RegAddr, XBH_U32 u32Length,  XBH_U8 *pu8Data)
{
...
XBH_S32 Gd32f330r8::ReadMcuMultiBytes(XBH_U8 s32RegAddr, XBH_U32 u32Length, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
 ...
 * 获取chip是否初始化完成
*/
XBH_S32 Gd32f330r8::getChipInitDone(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
 ...
 * 获取chip是否存在
*/
XBH_S32 Gd32f330r8::getChipExist(XBH_BOOL* enable)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    return  s32Ret;
}
...
//获取当前Mcu升级状态
XBH_S32 Gd32f330r8::getUpgradeState(XBH_S32* s32State)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    *s32State = mState;
...
XBH_S32 Gd32f330r8::WriteMcuByte(XBH_U8 s32RegAddr, XBH_U8* pu8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (!pu8Data)
    {
...
//2、mcu进入Iap模式
XBH_S32 Gd32f330r8::set_enter_iap_mode()
{
   XBH_S32 s32Ret = XBH_SUCCESS;
...
//3、写入mcu数据
XBH_S32 Gd32f330r8::upgrade_download_data(XBH_U8 *buff, XBH_U32 length)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 Gd32f330r8::calculate_CRC32_Mpeg2(void *pStart, unsigned int uiSize)
{
...
XBH_S32 Gd32f330r8::check_iap_CRC32(XBH_U32 pc_crc)
{
    XBH_S32 s32Ret = XBH_FAILURE;
...
XBH_S32 Gd32f330r8::set_iap_status(XBH_U8 status)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
//升级执行入口
XBH_S32 Gd32f330r8::MCUUpgrade(XBH_BOOL bForceUpgrade)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    ...
    XBH_BOOL check_crc = XBH_TRUE;
#if defined(XBH_BOARD_NAME)
    if(strcmp("XMH610A", XBH_BOARD_NAME) == 0) {
        unsigned char ver_buf[2] = {0x00};
        s32Ret = ReadMcuMultiBytes(CMD_I2C_GET_APP_VER, 2, ver_buf);
        if( s32Ret == XBH_SUCCESS) {
            XLOGE("crc mcu version= %d.%d", ver_buf[0], ver_buf[1]);
            if((ver_buf[0]<1) || (ver_buf[0]==1 && ver_buf[1]==0)) {
                // 小于1.0版本不校验
                check_crc = XBH_FALSE;
            }
        } else {
    ...
    if (check_iap_CRC32(u32FileCrc) != 1 && check_crc == XBH_TRUE)
    {
...
XBH_S32 Gd32f330r8::mcuReadBytes(XBH_MCU_I2CBUFFDEF_S* pI2cBuff)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
 ...
 * 获取MCU软件版本
 */
XBH_S32 Gd32f330r8::getMcuVersion(XBH_S32 *mcuVer)
{
 ...
 * 获取升级固件版本
 */
XBH_S32 Gd32f330r8::getFirmwareVersion(XBH_CHAR* strVersion)
{
    XBH_S32 s32Ret = XBH_FAILURE;
    XBH_S32 mcuVer = 0;
 ...
 * 进行固件升级
 */
XBH_S32 Gd32f330r8::upgradeFirmware(const XBH_CHAR* strFilePath, XBH_BOOL bForceUpgrade)
{
    XbhMutex::Autolock _l(mLock);
...
XBH_S32 Gd32f330r8::setMcuFattMode(XBH_S32 mode)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 pu8Data[1];
...
XBH_S32 Gd32f330r8::getMcuFattMode(XBH_S32 *mode)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 buf[64] = {0x00};
...
XBH_S32 Gd32f330r8::setMcuIIcBypass(XBH_U8 u8IIcNum, XBH_U8 u8DeviceAddr, XBH_U8 u8RegAddr, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[4] = {0x00};
    param[0] = u8IIcNum;
...
XBH_S32 Gd32f330r8::getMcuIIcBypass(XBH_U8 u8IIcNum, XBH_U8 u8DeviceAddr, XBH_U8 u8RegAddr, XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[4] = {0x00};
    param[0] = u8IIcNum;
...
XBH_S32 Gd32f330r8::setMcuUartBypassPara(XBH_U8 u8UartNum, XBH_U8 baud)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    XBH_U8 param[2] = {0x00};
    param[0] = u8UartNum;
...
XBH_S32 Gd32f330r8::setMcuUartBypass(XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
XBH_S32 Gd32f330r8::getMcuUartBypass(XBH_U8 u8Len, XBH_U8* u8Data)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
...
void Gd32f330r8::run(const void* arg)
{
    XBH_S32 s32Ret = XBH_SUCCESS;
    if (mUpgradeStatus == 1)
    {
...
Gd32f330r8::Gd32f330r8(XBH_S32 iicBus, XBH_S32 iicAddr, XBH_S32 pGpio, XBH_S32 pLevel, XBH_S32 rGpio, XBH_S32 rLevel)
{
    XLOGD(" begin ");
    mI2cBus = iicBus;
    mI2cAddr = iicAddr;
