...
dictionary *iniparser_load(const char *ininame)
{
    ...
    dictionary *dict;
    char acResolvedPath[PATH_MAX];
    memset(&acResolvedPath, 0, sizeof(acResolvedPath));

    realpath(ininame, acResolvedPath);
    if (0 == strlen(acResolvedPath)) {
        return NULL;
    }

    if ((in = fopen(acResolvedPath, "r")) == NULL) {
